...
export function NavBar({ items, className, activeUrl }: NavBarProps) {
  ...
  const resolvedActiveTab = useMemo(() => {
    if (!activeUrl) return activeTab;
    const activeItem = items.find((item) => item.url === activeUrl);
    return activeItem?.name ?? activeTab;
  }, [activeTab, activeUrl, items]);
    ...
    >
      <div className="flex items-center gap-3 rounded-full border border-border bg-background/5 px-1 py-1 shadow-lg backdrop-blur-lg">
        {items.map((item) => {
          ...
          const isActive = resolvedActiveTab === item.name;
